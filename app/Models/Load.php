...
class Load
{
    public static function all(array $filters = []): array
    {
        ...
        $sql = "SELECT l.*, c.customer_company_name, u.username AS driver_name
                FROM loads l
        ...
        if (!empty($filters['search'])) {
            $sql .= " AND (l.reference_number LIKE :search
    ...
    public static function find(int $id): ?array
    {
        ...
        $stmt = $pdo->prepare("SELECT l.*, c.customer_company_name, u.username AS driver_name
            FROM loads l
            JOIN customers c ON l.customer_id = c.id
            LEFT JOIN users u ON l.assigned_driver_id = u.id
            WHERE l.load_id = :id
    ...
    {
        $pdo = Database::pdo();
        $stmt = $pdo->prepare("UPDATE loads SET load_status = :s WHERE load_id = :id");
